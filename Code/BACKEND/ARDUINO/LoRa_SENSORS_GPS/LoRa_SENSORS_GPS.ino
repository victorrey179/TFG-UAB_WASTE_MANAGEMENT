...
void setup() {
  ...
  if(!display.begin(SSD1306_SWITCHCAPVCC, 0x3c, false, false)) { // 0x3C representa 128x32
    ...
    for(;;); // Si detecta el fallo anterior, detiene el código aquí­ hasta que se reinicie
  }
...
void loop() {
  ...
  //Lee la temperatura in grados Celsius

  if (isnan(h) || isnan(t)) {
    Serial.println("Failed reception");
    return;
    //Devuelve un error si no se detecta información
  }
  ...
  display.print("Transmitiendo (10s)");//Mensaje de confirmación
  display.drawBitmap(98, 44, 
                     logo_lora, 30, 19, WHITE);
  display.setTextSize(2);
  display.setCursor(0,45);
  display.print(Contador);//La cuenta actual que se enví­a 
  display.display();
